...
def fillObsCPT(bayesNet, gameState):
    ...
    # possible_houses = gameState.getPossibleHouses()
    # print possible_houses
    for house in gameState.getPossibleHouses():
        # print house
        for obsPos in gameState.getHouseWalls(house):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsFactor = bn.Factor([obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict()) # get a CPT
            rows = obsFactor.getAllPossibleAssignmentDicts() # get each rows of CPT above
            for row in rows:
                food_house_val = row[FOOD_HOUSE_VAR]
                ghost_house_val = row[GHOST_HOUSE_VAR]
                obs_val = row[obsVar]
                possibleAdjHouse = getPossibleAdjHouse(gameState, obsPos)
                if possibleAdjHouse != food_house_val and possibleAdjHouse != ghost_house_val:
                    if obs_val == BLUE_OBS_VAL:
                        obsFactor.setProbability(row, 0)
                    elif obs_val == RED_OBS_VAL:
                        obsFactor.setProbability(row, 0)
                    elif obs_val == NO_OBS_VAL:
                        obsFactor.setProbability(row, 1)
                elif possibleAdjHouse == food_house_val or ghost_house_val == food_house_val:
                    if obs_val == BLUE_OBS_VAL:
                        obsFactor.setProbability(row, 1 - PROB_FOOD_RED)
                    elif obs_val == RED_OBS_VAL:
                        obsFactor.setProbability(row, PROB_FOOD_RED)
                    elif obs_val == NO_OBS_VAL:
                        obsFactor.setProbability(row, 0)
                elif possibleAdjHouse == ghost_house_val:
                    if obs_val == BLUE_OBS_VAL:
                        obsFactor.setProbability(row, 1 - PROB_GHOST_RED)
                    elif obs_val == RED_OBS_VAL:
                        obsFactor.setProbability(row, PROB_GHOST_RED)
                    elif obs_val == NO_OBS_VAL:
                        obsFactor.setProbability(row, 0)
            bayesNet.setCPT(obsVar, obsFactor)
    # util.raiseNotDefined()

def getPossibleAdjHouse(gameState, obsPos):
    min = 99999
    adj = None
    index = None
    houses = gameState.getPossibleHouses()
    for i in range(len(houses)):
        d = util.manhattanDistance(houses[i], obsPos)
        if d < min:
            min = d
            adj = houses[i]
            index = i
    l = [BOTTOM_LEFT_VAL, TOP_LEFT_VAL, BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL]
    return l[index]
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    final_Fator = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    return reduce(lambda x,y: x if final_Fator.getProbability(x) > final_Fator.getProbability(y) else y, final_Fator.getAllPossibleAssignmentDicts())
    util.raiseNotDefined()
